...
def create_error_response(message, status_code):
    ...
    response = Response(
        json.dumps(error_data), status=status_code, mimetype="application/json"
    )
    return response
...
def get_views(article, year, month):
    ...
    if not re.match(r"^[\w\s-]+$", article):
        return create_error_response(
            "Invalid article name. Only alphanumeric characters are allowed.", 400
        )
    if not year.isdigit() or len(year) != 4:
        return create_error_response(
            "Invalid year. Year should be a 4-digit number.", 400
        )
    if not month.isdigit() or len(month) != 2 or int(month) < 1 or int(month) > 12:
        return create_error_response(
            "Invalid month. Month should be a 2-digit number between 01 and 12.", 400
        )
